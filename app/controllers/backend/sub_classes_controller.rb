module Backend
  class SubClassesController < Backend::ApplicationController
    ...
    def index
      ...
      @sub_classes = SubClass.select("sub_classes.*,main_classes.name as main_class_name").joins(:main_class).page(params[:id]).order('id desc')
    end
    ...
    def update
      if @sub_class.update(not_create_params)
    ...
    def create_params
      params.require(:sub_class).permit(:name, :sid, :pic_url, :main_class_id, :url, :desc )
    end
    def not_create_params
      params.require(:sub_class).permit(:name, :pic_url, :main_class_id, :url, :desc)
    end
